-- Create Table
IF OBJECT_ID('IndexUsage') IS NULL
BEGIN
    SELECT
    ...
    WHERE 1=0;
    CREATE CLUSTERED INDEX CIX_IndexUsage ON IndexUsage(collect_date) WITH(DATA_COMPRESSION=PAGE);
    CREATE INDEX NCIX_IndexUsage_IndexInfo ON IndexUsage(object_name, name) WITH(DATA_COMPRESSION=PAGE);
END
